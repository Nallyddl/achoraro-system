...
import PCBuilder from "./components/PCBuilder"; // <-- NUEVO
...
export default function App() {
  ...
  const handleAddPCConfig = (config: any) => {
    const pcProduct: Product = {
      id: `custom-pc-${Date.now()}`,
      title: "PC Gamer Personalizada",
      vendor: "Achorao Custom",
      price: config.totalPrice,
      image: "https://www.achorao.com/cdn/shop/files/pc_custom.jpg",
      available: true,
    };
    handleAddToCart(pcProduct);
    localStorage.setItem("last_pc_config", JSON.stringify(config));
  };
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] text-gray-200 flex flex-col font-sans antialiased overflow-x-hidden selection:bg-red-600/30 selection:text-white">
      <Header
        cartItems={cartItems}
      ...
      <main className="flex-1 pb-16">
        {/* PESTAÑA HOME (con todo el contenido original) */}
        {selectedTab === "home" && (
          <div className="space-y-12">
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-stone-950/80 via-transparent to-stone-950/30 flex items-end p-6 md:p-12">
                    <div className="max-w-xl space-y-2 md:space-y-4 text-left">
                      ...
                      <p className="text-zinc-300 text-xs sm:text-sm font-medium leading-relaxed">{slide.subtitle}</p>
                      <button
                        onClick={() => setSelectedTab(slide.linkTab)}
                        className="h-10 bg-red-600 hover:bg-red-500 text-white font-semibold text-xs px-6 rounded-full transition-all uppercase tracking-widest shadow-lg shadow-red-500/10 flex items-center gap-1.5 cursor-pointer"
                      >
                        {slide.buttonText}
                        <ArrowRight size={12} />
                      </button>
            ...
            <div className="w-full max-w-7xl mx-auto px-4">
              <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-white/10 bg-[#0F0F12] flex flex-col md:flex-row items-center p-8 md:p-12 gap-8">
                ...
                <div className="relative z-10 flex-1 space-y-3 text-center md:text-left">
                  <span className="text-xs bg-red-500/10 border border-red-500/20 text-red-400 px-3 py-1 rounded-full font-bold uppercase tracking-wider">
                    PROMO CTS ACTIVA 🔥
                  </span>
                ...
                <div className="relative z-10 flex items-center gap-2 font-display uppercase tracking-wider select-none text-white">
                  <div className="bg-black/85 border border-white/10 rounded-2xl w-14 sm:w-16 py-3 text-center">
                    <span className="block text-xl sm:text-2xl font-extrabold text-red-500 leading-none">05</span>
                    <span className="text-[10px] text-gray-500 font-bold mt-1 block">Días</span>
                  </div>
                  <div className="bg-black/85 border border-white/10 rounded-2xl w-14 sm:w-16 py-3 text-center">
                    <span className="block text-xl sm:text-2xl font-extrabold text-red-500 leading-none">12</span>
                    <span className="text-[10px] text-gray-500 font-bold mt-1 block">Horas</span>
                  </div>
                  <div className="bg-black/85 border border-white/10 rounded-2xl w-14 sm:w-16 py-3 text-center">
                    <span className="block text-xl sm:text-2xl font-extrabold text-red-500 leading-none">44</span>
                    <span className="text-[10px] text-gray-500 font-bold mt-1 block">Min</span>
                  </div>
                  <div className="bg-black/85 border border-white/10 rounded-2xl w-14 sm:w-16 py-3 text-center">
                    <span className="block text-xl sm:text-2xl font-extrabold text-red-500 leading-none">18</span>
            ...
            <div className="w-full max-w-7xl mx-auto px-4 space-y-6">
              <h3 className="text-lg font-bold uppercase border-l-4 border-red-500 pl-3 text-white tracking-tight">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  ...
                  >
                    <img src={item.img} alt={item.name} className="w-full h-full object-cover opacity-50 group-hover:scale-105 transition-transform duration-305" />
                    <div className="absolute inset-0 bg-black/40 flex items-end p-4">
                      <span className="font-bold text-xs uppercase tracking-wider text-white block group-hover:text-red-400 transition-colors">
                        {item.name}
                      </span>
                    </div>
                  </div>
            ...
            {/* Super Descuentos x24H */}
            <div className="w-full max-w-7xl mx-auto px-4 space-y-6">
              <div className="flex justify-between items-baseline">
                <h3 className="text-lg font-bold uppercase border-l-4 border-red-500 pl-3 text-white tracking-tight">
                  Super Descuentos x 24H 🔥
                </h3>
                <button onClick={() => setSelectedTab("products")} className="text-xs text-red-500 font-bold uppercase tracking-wider hover:underline cursor-pointer">
                  Ver Todo
                </button>
              </div>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {INITIAL_PRODUCTS.slice(1, 5).map((product) => (
                  <div key={product.id} className="bg-[#0F0F12] border border-white/5 hover:border-white/10 rounded-3xl p-4 space-y-3 transition-all relative flex flex-col justify-between">
                    {product.tag && (
                      <div className="absolute left-3 top-3 bg-red-600 text-white text-[9px] font-bold uppercase px-2 py-0.5 rounded z-10">
                        {product.tag}
                      </div>
                    )}
                    <div className="space-y-3">
                      <div className="h-44 bg-black/30 rounded-2xl overflow-hidden border border-white/5 flex items-center justify-center p-2 relative">
                        <img src={product.image} alt={product.title} className="h-full object-contain hover:scale-105 transition-transform duration-200" />
                      </div>
                      <div className="space-y-1">
                        <span className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">{product.vendor}</span>
                        <h4 className="text-xs font-semibold tracking-tight text-white line-clamp-2 h-8 leading-snug">{product.title}</h4>
                      </div>
                    </div>
                    <div className="pt-2 border-t border-white/5 flex items-center justify-between gap-2">
                      <div className="flex flex-col text-left font-mono">
                        {product.compareAtPrice && <span className="text-[10px] text-gray-500 line-through">S/. {product.compareAtPrice.toFixed(2)}</span>}
                        <span className="text-sm font-bold text-yellow-400">S/. {product.price.toFixed(2)}</span>
                      </div>
                      <button onClick={() => handleAddToCart(product)} className="bg-red-600 hover:bg-red-500 text-white text-[10px] uppercase font-bold tracking-wider px-3 h-8 rounded-xl transition-all shadow-md shadow-red-900/10 cursor-pointer">
                        Comprar
                      </button>
                    </div>
                  </div>
            ...
            {/* Producto destacado Gravastar con calculadora */}
              ...
              <div className="bg-[#0F0F12] border border-white/10 rounded-3xl p-6 sm:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-6 space-y-4">
                  <div className="relative bg-[#0A0A0B]/80 border border-white/5 aspect-square rounded-3xl flex items-center justify-center p-6">
                    <span className="absolute top-4 left-4 bg-red-600 text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full">OFERTA ESTELAR</span>
                    <img src={INITIAL_PRODUCTS[0].image} alt={INITIAL_PRODUCTS[0].title} className="max-h-[300px] object-contain hover:scale-105 transition-transform duration-300" />
                  </div>
                  <div className="grid grid-cols-4 gap-2">
                    {[INITIAL_PRODUCTS[0].image, INITIAL_PRODUCTS[0].secondaryImage, "https://www.achorao.com/cdn/shop/files/D_NQ_NP_852381-MPE86624203981_062025-O.webp?v=1763394327", "https://www.achorao.com/cdn/shop/files/m1proportada2.jpg?v=1763394327"].map((thumb, idx) => (
                ...
                <div className="lg:col-span-6 flex flex-col justify-between space-y-6">
                  <div className="space-y-3">
                    <span className="text-xs text-red-400 font-bold uppercase tracking-wider">{INITIAL_PRODUCTS[0].vendor}</span>
                    <h2 className="text-xl sm:text-2xl font-bold text-white tracking-tight leading-snug">{INITIAL_PRODUCTS[0].title}</h2>
                    <p className="text-xs text-gray-400 leading-relaxed font-semibold">Sensor óptico PixArt PAW 3395 de precisión ultra alta, ajustable hasta 26,000 DPI reales. Chasis de primera en polímero con diseño esquelético simétrico super-liviano de 72 gramos para sesiones prolongadas.</p>
                    <div className="grid grid-cols-2 gap-2 pt-2 text-[10px] text-gray-400 font-medium uppercase tracking-wider font-mono">
                      <div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>Modo Dual: Tipo C / 2.4G</div>
                      <div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>Switches 100M Clicks</div>
                      <div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>Software Programable</div>
                      <div className="flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></span>Batería 500mAh</div>
                    </div>
                  </div>
                  <ProductCalculator basePrice={INITIAL_PRODUCTS[0].price} onAddToCart={() => handleAddToCart(INITIAL_PRODUCTS[0])} />
                </div>
              </div>
            </div>
            ...
            <div className="w-full max-w-7xl mx-auto px-4" id="brands-accordion">
              <div className="bg-[#0F0F12] border border-white/10 rounded-3xl overflow-hidden shadow-xl">
                <button onClick={() => setIsBrandAccordionOpen(!isBrandAccordionOpen)} className="w-full p-6 flex flex-col sm:flex-row items-center justify-between text-center sm:text-left gap-4 bg-gradient-to-r from-red-950/20 via-[#0F0F12] to-black select-none text-white focus:outline-none cursor-pointer">
                  <div className="space-y-1">
                  ...
                  <div className="bg-red-600/10 border border-red-500/20 text-xs font-bold px-4 py-2 rounded-xl text-red-400 font-mono">
                ...
                {isBrandAccordionOpen && (
                  <div className="p-6 bg-[#0A0A0B]/70 border-t border-white/10 animate-in slide-in-from-top duration-200">
                    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-4">
                      {BRANDS.map((brand, i) => (
                        <div key={i} className="bg-[#0F0F12] border border-white/5 rounded-2xl p-4 flex flex-col items-center justify-center text-center gap-2">
                          <img src={brand.logo} alt={brand.name} className="h-12 w-12 rounded-xl object-contain border border-white/10 bg-black/40 p-1" />
                          <span className="text-[11px] font-bold text-gray-300 tracking-wide uppercase font-mono">{brand.name}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="w-full max-w-7xl mx-auto px-4">
              <div className="relative rounded-3xl overflow-hidden aspect-[16/9] md:aspect-[21/9] border border-white/10 flex items-center bg-black">
                <img src="https://www.achorao.com/cdn/shop/files/corsair-silla-gamer-sherwood-silla-gamer-corsair-tc500-luxe-840006678465-38400377454832.jpg?v=1754485024" alt="Oferta del día" className="absolute inset-0 w-full h-full object-cover opacity-60" />
                ...
                <div className="absolute left-[35%] top-[45%] group z-20">
                  <div className="relative flex items-center justify-center h-8 w-8">
                    ...
                    <div className="absolute left-6 top-1/2 -translate-y-1/2 hidden group-hover:block bg-[#0F0F12] border border-white/10 rounded-2xl p-4 w-44 shadow-2xl space-y-2 text-left">
                      <span className="text-[9px] text-red-400 font-bold uppercase tracking-wider block font-mono">DESTACADOS DE LUJO</span>
                      <h5 className="text-xs font-bold uppercase text-white tracking-wide leading-tight">Corsair TC500 LUXE</h5>
                      <span className="text-xs font-bold text-yellow-400 block font-mono">S/. 1,449.90</span>
                      <button onClick={() => handleAddToCart(INITIAL_PRODUCTS[9])} className="w-full h-7 bg-red-600 hover:bg-red-500 text-white font-semibold rounded-lg text-[9px] uppercase tracking-wider transition-colors cursor-pointer">
                        Comprar ahora
                      </button>
                    </div>
                  </div>
                ...
                <div className="relative z-10 max-w-sm sm:max-w-md p-6 sm:p-10 text-left space-y-2">
                  <span className="text-[10px] bg-red-600 text-white font-bold px-2.5 py-1 rounded-full uppercase tracking-widest inline-block font-mono">OFERTA DEL DÍA</span>
                  <h3 className="text-lg sm:text-2xl font-bold uppercase tracking-tight text-white leading-tight">Silla gamer Corsair TC500 LUXE</h3>
                  <p className="text-xs text-gray-300 leading-relaxed font-semibold">Diseño ergonómico premium para sesiones prolongadas de Sim Racing o trabajo corporativo intensivo.</p>
                  <button onClick={() => handleAddToCart(INITIAL_PRODUCTS[9])} className="h-9 bg-[#0F0F12] hover:bg-black border border-white/10 text-white text-[10px] font-bold uppercase rounded-xl px-5 transition-colors tracking-widest flex items-center gap-1.5 cursor-pointer">
                    ME LA LLEVO YA <ArrowRight size={10} className="text-red-400" />
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="w-full max-w-7xl mx-auto px-4 bg-[#0F0F12]/80 border border-white/10 rounded-3xl p-6 flex flex-col md:flex-row justify-between items-center gap-6">
              <div className="text-center md:text-left space-y-1">
                <span className="text-[10px] text-red-400 font-bold uppercase tracking-wider font-mono">TIENDA FÍSICA AUTORIZADA</span>
              ...
              <a href="https://maps.google.com?daddr=Av. Vía de Evitamiento 1639 - Ate" target="_blank" rel="noreferrer" className="h-11 bg-red-600 hover:bg-red-500 text-white font-semibold text-xs px-6 rounded-xl transition-all shadow-md flex items-center gap-1.5 whitespace-nowrap shadow-red-900/15">
                CÓMO LLEGAR (GOOGLE MAPS) <ArrowRight size={12} />
              </a>
            </div>
          </div>
        )}
        ...
        {selectedTab === "products" && (
          <div className="w-full max-w-7xl mx-auto px-4 py-8 space-y-8 animate-in fade-in duration-200">
            <div className="border-b border-white/5 pb-4 flex flex-col sm:flex-row justify-between items-center gap-4">
              <h3 className="text-xl font-bold uppercase border-l-4 border-red-500 pl-3 text-white tracking-tight">Catálogo de Productos Gaming</h3>
              <div className="text-xs font-mono text-gray-500">Mostrando {filteredProducts.length} productos</div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {filteredProducts.map((product) => (
                <div key={product.id} className="bg-[#0F0F12] border border-white/5 hover:border-white/10 rounded-3xl p-4 space-y-3 transition-all relative flex flex-col justify-between h-96 shadow-sm">
                  <div className="space-y-3">
                    <div className="h-44 bg-black/40 border border-white/5 rounded-2xl overflow-hidden flex items-center justify-center p-3 relative">
                      {product.tag && <div className="absolute left-2.5 top-2.5 bg-red-600 text-white text-[9px] font-bold uppercase px-2 py-0.5 rounded z-10">{product.tag}</div>}
                      <img src={product.image} alt={product.title} className="max-h-full object-contain" />
                    </div>
                    <div className="space-y-1">
                      <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">{product.vendor}</span>
                      <h4 className="text-xs font-semibold leading-normal text-white line-clamp-2">{product.title}</h4>
                    </div>
                  </div>
                  <div className="pt-2 border-t border-white/5 flex items-center justify-between gap-1">
                    <div className="flex flex-col text-left font-mono">
                      {product.compareAtPrice && <span className="text-[9px] text-gray-500 line-through">S/. {product.compareAtPrice.toFixed(2)}</span>}
                      <span className="text-xs font-bold text-yellow-400">S/. {product.price.toFixed(2)}</span>
                    </div>
                    <button onClick={() => handleAddToCart(product)} disabled={!product.available} className="bg-red-600 hover:bg-red-500 disabled:bg-gray-800 disabled:text-gray-600 text-[10px] font-bold uppercase px-3 h-8 rounded-xl transition-all shadow-md shadow-red-900/10 cursor-pointer">
        ...
        {/* PESTAÑA SIMULADOR DE MEJORA */}
        {selectedTab === "simulator" && <SimulatorConsole />}

        {/* PESTAÑA ARMA TU PC (NUEVA) */}
        {selectedTab === "builder" && <PCBuilder onAddToCart={handleAddPCConfig} />}
      </main>

      <Footer currentDistrict={selectedDistrict} />

      <LocationModal isOpen={isLocationModalOpen} onClose={() => setIsLocationModalOpen(false)} selectedDistrict={selectedDistrict} onSelectDistrict={handleSelectDistrict} />

      <CartDrawer isOpen={isCartDrawerOpen} onClose={() => setIsCartDrawerOpen(false)} cartItems={cartItems} onUpdateQty={handleUpdateQty} onRemoveItem={handleRemoveItem} onClearCart={() => saveCart([])} />
    </div>
  );
}